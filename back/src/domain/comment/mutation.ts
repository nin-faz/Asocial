...
export const addComment: NonNullable<MutationResolvers["addComment"]> = async (
  ...
  try {
    ...
    console.log("[MENTION] mentionSet=", [...mentionSet], "isHereMention=", isHereMention, "user=", user.username);
    ...
    if (isHereMention && user.username === "Nin") {
      try {
        ...
        console.log("[TOUS] isHereMention=true, user=", user.username, "allUsers count=", allUsers.length);
        ...
        console.log("[TOUS] done, notified", allUsers.length, "users");
      } catch (err) {
        console.error("[TOUS] error in @tous block:", err);
      }
    }
